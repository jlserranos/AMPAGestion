...
public class FacturaService
{
    ...
    public async Task<List<Factura>> GetTodasAsync(string? curso = null, string? busqueda = null,
        CategoriaGasto? categoria = null, bool? pagadas = null)
    {
        ...
        if (!string.IsNullOrEmpty(curso))
        {
            ...
            q = q.Where(f => f.Fecha >= desde && f.Fecha <= hasta);
        }
    ...
    {
        await using var db = await _factory.CreateDbContextAsync();
        return await db.Facturas.Include(f => f.Alertas).FirstOrDefaultAsync(f => f.Id == id);
    }
    ...
    {
        await using var db = await _factory.CreateDbContextAsync();
        var fechas  = await db.Facturas.Select(f => f.Fecha).ToListAsync();
