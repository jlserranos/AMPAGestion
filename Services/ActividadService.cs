...
public class ActividadService
{
    ...
    public async Task<List<Actividad>> GetTodasAsync(string? busqueda = null,
        bool? activas = null, string? curso = null)
    {
        ...
        if (!string.IsNullOrEmpty(curso))
        {
            ...
            q = q.Where(a => a.FechaInicio == null ||
                             (a.FechaInicio >= desde && a.FechaInicio <= hasta));
        }
    ...
    {
        await using var db = await _factory.CreateDbContextAsync();
        var fechas = await db.Actividades
            .Where(a => a.FechaInicio != null)
            .Select(a => a.FechaInicio!.Value).ToListAsync();
